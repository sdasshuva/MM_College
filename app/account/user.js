            ...
            items: [Ext.create('Ext.grid.Panel', {
                            ...
                            if (rec.id === UN.user) {
                                ...
                                Ext.Msg.show({
                                    ...
                                    fn: function(btn, text) {
                                        if (btn == 'yes') {
                                            socket.emit('DestroyUser', rec.id).on('DestroyUserReturn', function(message) {
                                                if (message == "success") {
                ...
                listeners: {
                    edit: function(column, rowIndex) {
                        ...
                        socket.emit('UpdateUser', rec).on('UpdateUserReturn', function(message) {
                            if (message == "success") {
                                if (Ext.getCmp('user_grid')) {
                                    Ext.getCmp('user_grid').getStore().load();
                                }
                ...
                handler: function() {
                    ...
                    if (form.isValid()) {
                        if(values.password==values.confirm){
                            values.id = rec.id
                            socket.emit('ChangePassword', values).on('ChangePasswordReturn', function(message) {
                                if (message == "success") {
                                    if (Ext.getCmp('user_grid')) {
                                        Ext.getCmp('user_grid').getStore().load();
                                    }
        ...
        items: [Ext.create('Ext.form.Panel', {
            ...
            buttons: [{
                ...
                handler: function() {
                    ...
                    if (form.isValid()) {
                        socket.emit('CreateUser', values).on('CreateUserReturn', function(message) {
                            if (message == "success") {
                                if (Ext.getCmp('user_grid')) {
                                    Ext.getCmp('user_grid').getStore().load();
                                }
        ...
        columns: [{
            ...
            items: [{
                ...
                handler: function(grid, rowIndex, colIndex) {
                    ...
                    socket.emit('CreateUserNavigation', values).on('CreateUserNavigationReturn', function(message) {
                        if (message == "success") {
    ...
    return Ext.create('Ext.grid.Panel', {
        ...
        columns: [{
            ...
            items: [{
                ...
                handler: function(grid, rowIndex, colIndex) {
                    var rec = grid.getStore().getAt(rowIndex);
                    socket.emit('DestroyUserNavigation', rec.id).on('DestroyUserNavigationReturn', function(message) {
                        if (message == "success") {
                            if (Ext.getCmp('userPermissionUnselectedGrid')) {
                                Ext.getCmp('userPermissionUnselectedGrid').getStore().load();
                            }
...
function updateUserNavRole(a) {
    var b = JSON.parse(a)
    socket.emit('UpdateUserNavigationRole', b).on('UpdateUserNavigationRoleReturn', function(message) {
        if (message == "success") {
            if (Ext.getCmp('userPermissionSelectedGrid')) {
                Ext.getCmp('userPermissionSelectedGrid').getStore().load();
            }
